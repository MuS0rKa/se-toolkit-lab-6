...
def test_agent_list_files_tool():
    ...
    wiki_calls = [
        tc for tc in list_files_calls if tc.get("args", {}).get("path") == "wiki"
    ]
...
def test_agent_read_file_tool():
    ...
    git_calls = [
        ...
        if "git-workflow.md" in tc.get("args", {}).get("path", "")
    ]

    assert has_git_workflow or len(git_calls) > 0, (
        "Expected reference to git-workflow.md"
    )
...
def test_agent_missing_question():
    ...
    result = subprocess.run(
        [sys.executable, "agent.py"], capture_output=True, text=True
    )
...
def test_path_traversal_security():
    ...
    for tc in output.get("tool_calls", []):
        if tc.get("tool") == "read_file":
            ...
            if (
                "path traversal" in result_content.lower()
                or "invalid path" in result_content.lower()
            ):
