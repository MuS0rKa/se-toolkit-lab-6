...
- "Risky operations in analytics.py?" -> read_file backend/app/routers/analytics.py, then explicitly mention BOTH: (1) ZeroDivisionError in /completion-rate: rate = (passed_learners / total_learners) * 100 crashes when total_learners=0, (2) TypeError in /top-learners: sorted(rows, key=lambda r: r.avg_score, reverse=True) causes TypeError/NoneType error when avg_score is None- "What error does /analytics/completion-rate return?" -> query_api GET /analytics/completion-rate?lab=lab-99 immediately, status 500 means ZeroDivisionError, then read_file backend/app/routers/analytics.py line with "rate = (passed_learners / total_learners)"- "What error does endpoint return?" -> query_api first, then read_file the relevant router
